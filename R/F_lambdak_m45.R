#' The lambdas for each coavariate for models 4 and 5 
#'
...
lambdak_gl <- function(times, subj, X, y, d, tau, kn, degree, lambda, gam){
  ...
  X = matrix(X, nrow = dim)
  px = ncol(X)
  qvc2_indiv = qrvcp_gl(times, subj, y, X, tau, kn = kn, degree = degree, 
                        lambda = 0, d,rep(1,px))$hat_bt
  range = NULL
  for (k in 1:px) {
    range [k] = (max(qvc2_indiv[seq((k - 1) * dim + 1, k * dim)]) -
                   min(qvc2_indiv[seq((k - 1) * dim + 1, k * dim)]))^(-(gam))
  }
  lambdasic = Lamb_gl(times, subj, X, y, d, tau, kn, degree, lambda,  range)$lambdasic
